...
foreach ($target === 'all' ? Php::VERSIONS : [$target] as $realTarget) {
    ...
    \file_put_contents('src/Composer/Plugin.php', \str_replace('Phabel\\Symfony', 'Symfony', \file_get_contents('src/Composer/Plugin.php')));
    \rename("vendor", "vendor-bundle");
    r("find src -type f -exec sed 's/\\\\Phabel\\\\self/self/g' -i {} +");
    \file_put_contents(
        'vendor-bundle/autoload.php',
        <<<PHP
    <?php
...
PHP
    ...
    $json['require-dev'] = ['bamarni/composer-bin-plugin' => $json['require-dev']['bamarni/composer-bin-plugin']];

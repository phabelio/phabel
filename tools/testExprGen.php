...
foreach (Php::VERSIONS as $version) {
    ...
    $promise->onResolve(function (?\Throwable $e, ?array $res) use ($version, &$packagesSecondary) {
        ...
        $packagesSecondary[] = TraverserTask::runAsync([PhabelTestGenerator::class => ['target' => 1000 + $version]], "testsGenerated/Target{$version}", "testsGenerated/Target10{$version}", "expr10{$version}");
    });
}

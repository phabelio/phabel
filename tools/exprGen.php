...
class ExpressionGenerator
{
    ...
    {
        if (!\is_int($version)) {
            if (!(\is_bool($version) || \is_numeric($version))) {
    ...
    private function checkSyntax($code, $startFrom = 56)
    {
        ...
        if (!\is_int($startFrom)) {
            if (!(\is_bool($startFrom) || \is_numeric($startFrom))) {
...
PHP;
        ...
        foreach ($instanceArgTypes as $class => $argTypes) {
            ...
            foreach ($argTypes as $key => $phabel_67053abfb89d9db2) {
                $isArray = $phabel_67053abfb89d9db2[0];
                $types = $phabel_67053abfb89d9db2[1];

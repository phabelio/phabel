    ...
    protected function postprocessTokens(ErrorHandler $errorHandler)
    {
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            ...
            if (\Phabel\Target\Php80\Polyfill::substr($this->code, $filePos, $tokenLen) !== $tokenValue) {
                ...
                $nextFilePos = \Phabel\Target\Php71\Polyfill::strpos($this->code, $tokenValue, $filePos);
            ...
            $filePos += $tokenLen;
            $line += \Phabel\Target\Php71\Polyfill::substr_count($tokenValue, "\n");
        }
        if ($filePos !== \strlen($this->code)) {
            if (\Phabel\Target\Php80\Polyfill::substr($this->code, $filePos, 2) === '/*') {
                ...
                $errorHandler->handleError(new Error('Unterminated comment', ['startLine' => $line, 'endLine' => $line + \Phabel\Target\Php71\Polyfill::substr_count($comment, "\n"), 'startFilePos' => $filePos, 'endFilePos' => $filePos + \strlen($comment)]));
        ...
        if (\count($this->tokens) > 0) {
            ...
            if ($this->isUnterminatedComment($lastToken)) {
                $errorHandler->handleError(new Error('Unterminated comment', ['startLine' => $line - \Phabel\Target\Php71\Polyfill::substr_count($lastToken[1], "\n"), 'endLine' => $line, 'startFilePos' => $filePos - \strlen($lastToken[1]), 'endFilePos' => $filePos]));
            }
        }
    }
    /**
        ...
        while (1) {
            ...
            if (\is_string($token)) {
                ...
                if (\T_CLOSE_TAG === $token[0]) {
                    $this->prevCloseTagHasNewline = \false !== \Phabel\Target\Php71\Polyfill::strpos($token[1], "\n");
                } elseif (\T_INLINE_HTML === $token[0]) {
                    $startAttributes['hasLeadingNewline'] = $this->prevCloseTagHasNewline;
                }
                $this->line += \Phabel\Target\Php71\Polyfill::substr_count($value, "\n");
                $this->filePos += \strlen($value);
            } else {
                $origLine = $this->line;
                $origFilePos = $this->filePos;
                $this->line += \Phabel\Target\Php71\Polyfill::substr_count($token[1], "\n");

...
class Lexer
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $usedAttributes = \array_fill_keys(isset($options['usedAttributes']) ? $options['usedAttributes'] : $defaultAttributes, \true);
    ...
    {
        $phabelReturn = ($token[0] === \T_COMMENT || $token[0] === \T_DOC_COMMENT) && \substr($token[1], 0, 2) === '/*' && \substr($token[1], -2) !== '*/';
    ...
    public function handleHaltCompiler()
    {
        ...
        $phabelReturn = \substr($textAfter, \strlen($matches[0]));
    ...
    private function defineCompatibilityTokens()
    {
        ...
        foreach ($compatTokens as $token) {
            if (\defined($token)) {
                ...
                $clashingToken = isset($usedTokenIds[$tokenId]) ? $usedTokenIds[$tokenId] : null;

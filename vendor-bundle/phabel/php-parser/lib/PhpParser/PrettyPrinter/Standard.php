...
namespace Phabel\PhpParser\PrettyPrinter;
...
use Phabel\PhpParser\PrettyPrinterAbstract;
class Standard extends PrettyPrinterAbstract
{
    ...
    protected function pAttrGroups(array $nodes, $inline = \false)
    {
        if (!\is_bool($inline)) {
            if (!(\is_bool($inline) || \is_numeric($inline) || \is_string($inline))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($inline) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($inline) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $inline = (bool) $inline;
            }
        }

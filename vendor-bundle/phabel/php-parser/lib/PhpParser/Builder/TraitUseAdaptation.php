    ...
    protected function setModifier($modifier)
    {
        if (!\is_int($modifier)) {
            if (!(\is_bool($modifier) || \is_numeric($modifier))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($modifier) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($modifier) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    public function getNode()
    {
        switch ($this->type) {
            case self::TYPE_ALIAS:
                $phabelReturn = new Stmt\TraitUseAdaptation\Alias($this->trait, $this->method, $this->modifier, $this->alias);
            ...
            case self::TYPE_PRECEDENCE:
                $phabelReturn = new Stmt\TraitUseAdaptation\Precedence($this->trait, $this->method, $this->insteadof);
                if (!$phabelReturn instanceof Node) {
                    throw new \TypeError(__METHOD__ . '(): Return value must be of type Node, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                }
                return $phabelReturn;

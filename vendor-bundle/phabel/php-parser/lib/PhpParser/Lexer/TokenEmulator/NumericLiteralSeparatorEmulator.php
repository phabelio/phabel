...
final class NumericLiteralSeparatorEmulator extends TokenEmulator
{
    ...
    private function resolveIntegerOrFloatToken(string $str) : int
    {
        ...
        if (\Phabel\Target\Php71\Polyfill::stripos($str, '0b') === 0) {
            $num = \bindec($str);
        } elseif (\Phabel\Target\Php71\Polyfill::stripos($str, '0x') === 0) {
            $num = \hexdec($str);
        } elseif (\Phabel\Target\Php71\Polyfill::stripos($str, '0') === 0 && \ctype_digit($str)) {
            $num = \octdec($str);
        } else {
            $num = +$str;
        }

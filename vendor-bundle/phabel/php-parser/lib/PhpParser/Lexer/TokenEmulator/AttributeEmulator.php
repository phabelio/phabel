...
final class AttributeEmulator extends TokenEmulator
{
    ...
    {
        return \Phabel\Target\Php71\Polyfill::strpos($code, '#[') !== \false;
    }
    public function emulate(string $code, array $tokens) : array
    {
        ...
        for ($i = 0, $c = \count($tokens); $i < $c; ++$i) {
            ...
            if (\is_array($tokens[$i])) {
                $line += \Phabel\Target\Php71\Polyfill::substr_count($tokens[$i][1], "\n");
            }
        }
        return $tokens;
    }
    ...
    {
        $pos = 0;
        while (\false !== ($pos = \Phabel\Target\Php71\Polyfill::strpos($code, '#[', $pos))) {
            // Replace #[ with %[
            $code[$pos] = '%';
            $patches[] = [$pos, 'replace', '#'];
            $pos += 2;

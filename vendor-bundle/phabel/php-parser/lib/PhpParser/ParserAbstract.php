...
use Phabel\PhpParser\Node\Scalar\Encapsed;
...
use Phabel\PhpParser\Node\Stmt\Namespace_;
...
abstract class ParserAbstract implements Parser
{
    ...
    {
        if (!\is_int($symbol)) {
            if (!(\is_bool($symbol) || \is_numeric($symbol))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($symbol) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($symbol) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $symbol = (int) $symbol;
            }
        }
        if (!\is_int($state)) {
            if (!(\is_bool($state) || \is_numeric($state))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($state) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($state) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $state = (int) $state;
            }
        }
        ...
        $phabelReturn = 'Syntax error, unexpected ' . $this->symbolToName[$symbol] . $expectedString;
    ...
    */
    ...
    protected function handleNamespaces(array $stmts)
    {
        ...
        if (null === $style) {
            ...
            $phabelReturn = $resultStmts;
            if (!\is_array($phabelReturn)) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type array, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            }
            return $phabelReturn;
        }
        throw new \TypeError(__METHOD__ . '(): Return value must be of type array, none returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
    }
    ...
    protected function fixupPhp5StaticPropCall($prop, array $args, array $attributes)
    {
        if ($prop instanceof Node\Expr\StaticPropertyFetch) {
            ...
            $phabelReturn = new Expr\StaticCall($prop->class, $var, $args, $attributes);
            ...
            $phabelReturn = new Expr\StaticCall($staticProp->class, $prop, $args, $attributes);
            if (!$phabelReturn instanceof Expr\StaticCall) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type Expr\\StaticCall, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            }
            return $phabelReturn;
        ...
        throw new \TypeError(__METHOD__ . '(): Return value must be of type Expr\\StaticCall, none returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
    }
    ...
    protected function getAttributesAt($pos)
    {
        ...
        $phabelReturn = $this->startAttributeStack[$pos] + $this->endAttributeStack[$pos];
        if (!\is_array($phabelReturn)) {
            throw new \TypeError(__METHOD__ . '(): Return value must be of type array, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
        return $phabelReturn;
    }
    protected function getFloatCastKind($cast)
    {
        if (!\is_string($cast)) {
            if (!(\is_string($cast) || \is_object($cast) && \method_exists($cast, '__toString') || (\is_bool($cast) || \is_numeric($cast)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($cast) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($cast) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $cast = (string) $cast;
            }
        }
        ...
        if (\strpos($cast, 'float') !== \false) {
            $phabelReturn = Double::KIND_FLOAT;
        ...
        $phabelReturn = Double::KIND_DOUBLE;
        if (!\is_int($phabelReturn)) {
            if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    protected function stripIndentation($string, $indentLen, $indentChar, $newlineAtStart, $newlineAtEnd, array $attributes)
    {
        if (!\is_string($string)) {
        ...
        if (!\is_int($indentLen)) {
            if (!(\is_bool($indentLen) || \is_numeric($indentLen))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($indentLen) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($indentLen) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $indentLen = (int) $indentLen;
            }
        }
        if (!\is_string($indentChar)) {
            if (!(\is_string($indentChar) || \is_object($indentChar) && \method_exists($indentChar, '__toString') || (\is_bool($indentChar) || \is_numeric($indentChar)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #3 ($indentChar) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($indentChar) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $indentChar = (string) $indentChar;
            }
        }
        if (!\is_bool($newlineAtStart)) {
            if (!(\is_bool($newlineAtStart) || \is_numeric($newlineAtStart) || \is_string($newlineAtStart))) {
                throw new \TypeError(__METHOD__ . '(): Argument #4 ($newlineAtStart) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($newlineAtStart) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $newlineAtStart = (bool) $newlineAtStart;
            }
        }
        if (!\is_bool($newlineAtEnd)) {
            if (!(\is_bool($newlineAtEnd) || \is_numeric($newlineAtEnd) || \is_string($newlineAtEnd))) {
                throw new \TypeError(__METHOD__ . '(): Argument #5 ($newlineAtEnd) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($newlineAtEnd) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $newlineAtEnd = (bool) $newlineAtEnd;
            }
        }
        if ($indentLen === 0) {
            return $string;
    ...
    protected function parseDocString($startToken, $contents, $endToken, array $attributes, array $endTokenAttributes, $parseUnicodeEscape)
    {
        if (!\is_string($startToken)) {
            if (!(\is_string($startToken) || \is_object($startToken) && \method_exists($startToken, '__toString') || (\is_bool($startToken) || \is_numeric($startToken)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($startToken) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($startToken) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $startToken = (string) $startToken;
            }
        }
        if (!\is_string($endToken)) {
            if (!(\is_string($endToken) || \is_object($endToken) && \method_exists($endToken, '__toString') || (\is_bool($endToken) || \is_numeric($endToken)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #3 ($endToken) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($endToken) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
        ...
        if (!\is_bool($parseUnicodeEscape)) {
            if (!(\is_bool($parseUnicodeEscape) || \is_numeric($parseUnicodeEscape) || \is_string($parseUnicodeEscape))) {
                throw new \TypeError(__METHOD__ . '(): Argument #6 ($parseUnicodeEscape) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($parseUnicodeEscape) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {

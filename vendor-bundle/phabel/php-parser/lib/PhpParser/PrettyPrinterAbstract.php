...
use Phabel\PhpParser\Internal\DiffElem;
use Phabel\PhpParser\Internal\PrintableNewAnonClassNode;
use Phabel\PhpParser\Internal\TokenStream;
...
abstract class PrettyPrinterAbstract
{
    ...
    public function prettyPrint(array $stmts)
    {
        ...
        $phabelReturn = \ltrim($this->handleMagicTokens($this->pStmts($stmts, \false)));
    ...
    {
        $this->resetState();
        $phabelReturn = $this->handleMagicTokens($this->p($node));
        if (!\is_string($phabelReturn)) {
            if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    {
        if (!\is_bool($indent)) {
            if (!(\is_bool($indent) || \is_numeric($indent) || \is_string($indent))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($indent) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($indent) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $indent = (bool) $indent;
            }
        }
     ...
     */
    protected function pInfixOp($class, Node $leftNode, $operatorString, Node $rightNode)
    {
        if (!\is_string($class)) {
        ...
        if (!\is_string($operatorString)) {
            if (!(\is_string($operatorString) || \is_object($operatorString) && \method_exists($operatorString, '__toString') || (\is_bool($operatorString) || \is_numeric($operatorString)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #3 ($operatorString) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($operatorString) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $operatorString = (string) $operatorString;
            }
        }
        list($precedence, $associativity) = $this->precedenceMap[$class];
        $phabelReturn = $this->pPrec($leftNode, $precedence, $associativity, -1) . $operatorString . $this->pPrec($rightNode, $precedence, $associativity, 1);
        if (!\is_string($phabelReturn)) {
            if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
     ...
     */
    protected function pPrefixOp($class, $operatorString, Node $node)
    {
        if (!\is_string($class)) {
            if (!(\is_string($class) || \is_object($class) && \method_exists($class, '__toString') || (\is_bool($class) || \is_numeric($class)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($class) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($class) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        ...
        if (!\is_string($operatorString)) {
            if (!(\is_string($operatorString) || \is_object($operatorString) && \method_exists($operatorString, '__toString') || (\is_bool($operatorString) || \is_numeric($operatorString)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($operatorString) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($operatorString) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $operatorString = (string) $operatorString;
            }
        }
        list($precedence, $associativity) = $this->precedenceMap[$class];
        $phabelReturn = $operatorString . $this->pPrec($node, $precedence, $associativity, 1);
        if (!\is_string($phabelReturn)) {
            if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
     ...
     */
    protected function pPostfixOp($class, Node $node, $operatorString)
    {
        if (!\is_string($class)) {
            if (!(\is_string($class) || \is_object($class) && \method_exists($class, '__toString') || (\is_bool($class) || \is_numeric($class)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($class) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($class) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        ...
        list($precedence, $associativity) = $this->precedenceMap[$class];
        $phabelReturn = $this->pPrec($node, $precedence, $associativity, -1) . $operatorString;
        if (!\is_string($phabelReturn)) {
            if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    {
        if (!\is_int($parentPrecedence)) {
            if (!(\is_bool($parentPrecedence) || \is_numeric($parentPrecedence))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($parentPrecedence) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($parentPrecedence) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $parentPrecedence = (int) $parentPrecedence;
            }
        }
        if (!\is_int($parentAssociativity)) {
            if (!(\is_bool($parentAssociativity) || \is_numeric($parentAssociativity))) {
                throw new \TypeError(__METHOD__ . '(): Argument #3 ($parentAssociativity) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($parentAssociativity) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $parentAssociativity = (int) $parentAssociativity;
            }
        }
        if (!\is_int($childPosition)) {
            if (!(\is_bool($childPosition) || \is_numeric($childPosition))) {
                throw new \TypeError(__METHOD__ . '(): Argument #4 ($childPosition) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($childPosition) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    {
        if (!\is_string($glue)) {
            if (!(\is_string($glue) || \is_object($glue) && \method_exists($glue, '__toString') || (\is_bool($glue) || \is_numeric($glue)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($glue) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($glue) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $glue = (string) $glue;
            }
        }
        ...
        $phabelReturn = \implode($glue, $pNodes);
        if (!\is_string($phabelReturn)) {
            if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    {
        $phabelReturn = $this->pImplode($nodes, ', ');
        if (!\is_string($phabelReturn)) {
            if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    {
        if (!\is_bool($trailingComma)) {
            if (!(\is_bool($trailingComma) || \is_numeric($trailingComma) || \is_string($trailingComma))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($trailingComma) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($trailingComma) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $trailingComma = (bool) $trailingComma;
            }
        }
        $this->indent();
        $result = '';
    ...
    protected function pComments(array $comments)
    {
        ...
        $phabelReturn = \implode($this->nl, $formattedComments);
        if (!\is_string($phabelReturn)) {
            if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    public function printFormatPreserving(array $stmts, array $origStmts, array $origTokens)
    {
        ...
        $phabelReturn = \ltrim($this->handleMagicTokens($result));
        if (!\is_string($phabelReturn)) {
            if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
     ...
     * @return string Pretty printed node
     */
    protected function p(Node $node, $parentFormatPreserved = \false)
    {
        // No orig tokens means this is a normal pretty print without preservation of formatting
        if (!$this->origTokens) {
            $phabelReturn = $this->{'p' . $node->getType()}($node);
            if (!\is_string($phabelReturn)) {
                if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                    throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                } else {
        ...
        if (null === $origNode) {
            $phabelReturn = $this->pFallback($node);
            if (!\is_string($phabelReturn)) {
                if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                    throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                } else {
        ...
        if ($node instanceof Stmt\InlineHTML && !$parentFormatPreserved) {
            $phabelReturn = $this->pFallback($fallbackNode);
            if (!\is_string($phabelReturn)) {
                if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                    throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                } else {
        ...
        $type = $node->getType();
        $fixupInfo = isset($this->fixupMap[$class]) ? $this->fixupMap[$class] : null;
        $result = '';
        $pos = $startPos;
        foreach ($node->getSubNodeNames() as $subNodeName) {
            ...
            if (!$subNode instanceof Node && $subNode !== null || !$origSubNode instanceof Node && $origSubNode !== null) {
                ...
                if (\is_array($subNode) && \is_array($origSubNode)) {
                    ...
                    $listResult = $this->pArray($subNode, $origSubNode, $pos, $indentAdjustment, $type, $subNodeName, isset($fixupInfo[$subNodeName]) ? $fixupInfo[$subNodeName] : null);
                    if (null === $listResult) {
                        $phabelReturn = $this->pFallback($fallbackNode);
                        if (!\is_string($phabelReturn)) {
                            if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
     ...
     */
    protected function pArray(array $nodes, array $origNodes, &$pos, $indentAdjustment, $parentNodeType, $subNodeName, $fixup)
    {
        if (!\is_int($pos)) {
            if (!(\is_bool($pos) || \is_numeric($pos))) {
                throw new \TypeError(__METHOD__ . '(): Argument #3 ($pos) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($pos) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $pos = (int) $pos;
            }
        }
        if (!\is_int($indentAdjustment)) {
            if (!(\is_bool($indentAdjustment) || \is_numeric($indentAdjustment))) {
                throw new \TypeError(__METHOD__ . '(): Argument #4 ($indentAdjustment) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($indentAdjustment) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $indentAdjustment = (int) $indentAdjustment;
            }
        }
        if (!\is_string($parentNodeType)) {
            if (!(\is_string($parentNodeType) || \is_object($parentNodeType) && \method_exists($parentNodeType, '__toString') || (\is_bool($parentNodeType) || \is_numeric($parentNodeType)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #5 ($parentNodeType) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($parentNodeType) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $parentNodeType = (string) $parentNodeType;
            }
        }
        if (!\is_string($subNodeName)) {
            if (!(\is_string($subNodeName) || \is_object($subNodeName) && \method_exists($subNodeName, '__toString') || (\is_bool($subNodeName) || \is_numeric($subNodeName)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #6 ($subNodeName) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($subNodeName) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $subNodeName = (string) $subNodeName;
            }
        }
        $diff = $this->nodeListDiffer->diffWithReplacements($origNodes, $nodes);
        $mapKey = $parentNodeType . '->' . $subNodeName;
        $insertStr = isset($this->listInsertionMap[$mapKey]) ? $this->listInsertionMap[$mapKey] : null;
     ...
     */
    protected function pFixup($fixup, Node $subNode, $parentClass, $subStartPos, $subEndPos)
    {
        if (!\is_int($fixup)) {
            if (!(\is_bool($fixup) || \is_numeric($fixup))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($fixup) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($fixup) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $fixup = (int) $fixup;
            }
        }
        if (!\is_int($subStartPos)) {
            if (!(\is_bool($subStartPos) || \is_numeric($subStartPos))) {
                throw new \TypeError(__METHOD__ . '(): Argument #4 ($subStartPos) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($subStartPos) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $subStartPos = (int) $subStartPos;
            }
        }
        if (!\is_int($subEndPos)) {
            if (!(\is_bool($subEndPos) || \is_numeric($subEndPos))) {
                throw new \TypeError(__METHOD__ . '(): Argument #5 ($subEndPos) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($subEndPos) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $subEndPos = (int) $subEndPos;
            }
        }
        switch ($fixup) {
            ...
            case self::FIXUP_PREC_RIGHT:
                if (!$this->origTokens->haveParens($subStartPos, $subEndPos)) {
                    ...
                    $phabelReturn = $this->pPrec($subNode, $precedence, $associativity, $fixup === self::FIXUP_PREC_LEFT ? -1 : 1);
                    if (!\is_string($phabelReturn)) {
                        if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                            throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                        } else {
            ...
            case self::FIXUP_VAR_BRACED_NAME:
                if ($subNode instanceof Expr && !$this->origTokens->haveBraces($subStartPos, $subEndPos)) {
                    $phabelReturn = ($fixup === self::FIXUP_VAR_BRACED_NAME ? '$' : '') . '{' . $this->p($subNode) . '}';
                    if (!\is_string($phabelReturn)) {
                        if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                            throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                        } else {
    ...
    protected function safeAppend(&$str, $append)
    {
        ...
        if (!\is_string($append)) {
            if (!(\is_string($append) || \is_object($append) && \method_exists($append, '__toString') || (\is_bool($append) || \is_numeric($append)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($append) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($append) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    {
        $phabelReturn = !($node instanceof Node\Name || $node instanceof Expr\Variable || $node instanceof Expr\ArrayDimFetch || $node instanceof Expr\FuncCall || $node instanceof Expr\MethodCall || $node instanceof Expr\NullsafeMethodCall || $node instanceof Expr\StaticCall || $node instanceof Expr\Array_);

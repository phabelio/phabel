...
class UnicodeString extends AbstractUnicodeString
{
    ...
    public function endsWith($suffix) : bool
    {
        ...
        if ($this->ignoreCase) {
            return 0 === \Phabel\Target\Php71\Polyfill::mb_stripos(\grapheme_extract($this->string, \strlen($suffix), \GRAPHEME_EXTR_MAXBYTES, \strlen($this->string) - \strlen($suffix)), $suffix, 0, 'UTF-8');
        }
    ...
    public function equalsTo($string) : bool
    {
        ...
        if ('' !== $string && \false !== $string && $this->ignoreCase) {
            return \strlen($string) === \strlen($this->string) && 0 === \Phabel\Target\Php71\Polyfill::mb_stripos($this->string, $string, 0, 'UTF-8');
        }
    ...
    public function indexOf($needle, int $offset = 0)
    {
        ...
        try {
            $i = $this->ignoreCase ? \Phabel\Target\Php71\Polyfill::grapheme_stripos($this->string, $needle, $offset) : \Phabel\Target\Php71\Polyfill::grapheme_strpos($this->string, $needle, $offset);
        } catch (\ValueError $e) {
            $phabelReturn = null;
            if (!\is_null($phabelReturn)) {
                if (!\is_int($phabelReturn)) {
    ...
    public function startsWith($prefix) : bool
    {
        ...
        if ($this->ignoreCase) {
            return 0 === \Phabel\Target\Php71\Polyfill::mb_stripos(\grapheme_extract($this->string, \strlen($prefix), \GRAPHEME_EXTR_MAXBYTES), $prefix, 0, 'UTF-8');
        }

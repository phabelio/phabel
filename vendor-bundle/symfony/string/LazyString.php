                                ...
                                if (!(\is_bool($value) || \is_numeric($value))) {
                                    if (!\is_string($value)) {
                                        if (!(\is_string($value) || \Phabel\Target\Php72\Polyfill::is_object($value) && \method_exists($value, '__toString') || (\is_bool($value) || \is_numeric($value)))) {
                                            throw new \TypeError(__METHOD__ . '(): Argument #1 ($value) must be of type Stringable|string|int|float|bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($value) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                                        } else {
                                            $value = (string) $value;
                                        }
                                    }

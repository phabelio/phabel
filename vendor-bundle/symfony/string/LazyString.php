...
class LazyString implements \Stringable, \JsonSerializable
{
    ...
    public static function fromStringable($value) : self
    {
        ...
        if (\Phabel\Target\Php72\Polyfill::is_object($value)) {
    ...
    {
        return \is_string($value) || $value instanceof self || (\Phabel\Target\Php72\Polyfill::is_object($value) ? \method_exists($value, '__toString') : \is_scalar($value));
    }
    /**
    ...
    private static function getPrettyName(callable $callback) : string
    {
        ...
        if (\is_array($callback)) {
            $class = \Phabel\Target\Php72\Polyfill::is_object($callback[0]) ? \get_debug_type($callback[0]) : $callback[0];
            $method = $callback[1];
        } elseif ($callback instanceof \Closure) {
            $r = new \ReflectionFunction($callback);
            if (\false !== \Phabel\Target\Php71\Polyfill::strpos($r->name, '{closure}') || !($class = $r->getClosureScopeClass())) {
                return $r->name;
            }
            $class = $class->name;
            $method = $r->name;

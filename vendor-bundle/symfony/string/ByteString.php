...
class ByteString extends AbstractString
{
    ...
    {
        if (!(\is_array($suffix) || $suffix instanceof \Traversable || $suffix instanceof AbstractString)) {
            if (!\is_string($suffix)) {
    ...
    {
        if (!(\is_array($string) || $string instanceof \Traversable || $string instanceof AbstractString)) {
            if (!\is_string($string)) {
    ...
    {
        if (!(\is_array($needle) || $needle instanceof \Traversable || $needle instanceof AbstractString)) {
            if (!\is_string($needle)) {
        ...
        try {
            if (\false === $match($regexp, $this->string, $matches, $flags | 0, $offset)) {
                ...
                foreach (\get_defined_constants(\true)['pcre'] as $k => $v) {
                    if ($lastError === $v && '_ERROR' === \Phabel\Target\Php80\Polyfill::substr($k, -6)) {
    ...
    public function replaceMatches(string $fromRegexp, $to)
    {
        if (!\is_callable($to)) {
            if (!\is_string($to)) {
                if (!(\is_string($to) || \Phabel\Target\Php72\Polyfill::is_object($to) && \method_exists($to, '__toString') || (\is_bool($to) || \is_numeric($to)))) {
    ...
    {
        if (!(\is_array($prefix) || $prefix instanceof \Traversable || $prefix instanceof AbstractString)) {
            if (!\is_string($prefix)) {
    ...
    public function toCodePointString(string $fromEncoding = null) : CodePointString
    {
        ...
        $u->string = \Phabel\Target\Php72\Polyfill::mb_convert_encoding($this->string, 'UTF-8', $fromEncoding ?? 'Windows-1252');
        return $u;
    }
    /**
     *

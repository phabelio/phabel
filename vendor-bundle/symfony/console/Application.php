...
use Phabel\Symfony\Component\Console\Command\HelpCommand;
use Phabel\Symfony\Component\Console\Command\LazyCommand;
use Phabel\Symfony\Component\Console\Command\ListCommand;
use Phabel\Symfony\Component\Console\Command\SignalableCommandInterface;
use Phabel\Symfony\Component\Console\CommandLoader\CommandLoaderInterface;
use Phabel\Symfony\Component\Console\Event\ConsoleCommandEvent;
use Phabel\Symfony\Component\Console\Event\ConsoleErrorEvent;
use Phabel\Symfony\Component\Console\Event\ConsoleSignalEvent;
use Phabel\Symfony\Component\Console\Event\ConsoleTerminateEvent;
use Phabel\Symfony\Component\Console\Exception\CommandNotFoundException;
use Phabel\Symfony\Component\Console\Exception\ExceptionInterface;
use Phabel\Symfony\Component\Console\Exception\LogicException;
use Phabel\Symfony\Component\Console\Exception\NamespaceNotFoundException;
...
use Phabel\Symfony\Component\Console\Helper\DebugFormatterHelper;
use Phabel\Symfony\Component\Console\Helper\FormatterHelper;
use Phabel\Symfony\Component\Console\Helper\Helper;
use Phabel\Symfony\Component\Console\Helper\HelperSet;
use Phabel\Symfony\Component\Console\Helper\ProcessHelper;
use Phabel\Symfony\Component\Console\Helper\QuestionHelper;
use Phabel\Symfony\Component\Console\Input\ArgvInput;
use Phabel\Symfony\Component\Console\Input\ArrayInput;
use Phabel\Symfony\Component\Console\Input\InputArgument;
use Phabel\Symfony\Component\Console\Input\InputAwareInterface;
...
use Phabel\Symfony\Component\Console\SignalRegistry\SignalRegistry;
use Phabel\Symfony\Component\Console\Style\SymfonyStyle;
use Phabel\Symfony\Component\ErrorHandler\ErrorHandler;
use Phabel\Symfony\Contracts\EventDispatcher\EventDispatcherInterface;
use Phabel\Symfony\Contracts\Service\ResetInterface;
/**
...
class Application implements ResetInterface
{
    ...
    public function getSignalRegistry()
    {
        ...
        $phabelReturn = $this->signalRegistry;
        if (!$phabelReturn instanceof SignalRegistry) {
            throw new \TypeError(__METHOD__ . '(): Return value must be of type SignalRegistry, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
        return $phabelReturn;
    }
    public function setSignalsToDispatchEvent(...$signalsToDispatchEvent)
    {
        foreach ($signalsToDispatchEvent as $phabelVariadicIndex => $phabelVariadic) {
            if (!\is_int($phabelVariadic)) {
                if (!(\is_bool($phabelVariadic) || \is_numeric($phabelVariadic))) {
                    throw new \TypeError(__METHOD__ . '(): Argument #' . (1 + $phabelVariadicIndex) . ' must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($signalsToDispatchEvent) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                } else {
    ...
    {
        if (!\is_bool($boolean)) {
            if (!(\is_bool($boolean) || \is_numeric($boolean) || \is_string($boolean))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($boolean) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($boolean) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    public function findNamespace($namespace)
    {
        if (!\is_string($namespace)) {
            if (!(\is_string($namespace) || \is_object($namespace) && \method_exists($namespace, '__toString') || (\is_bool($namespace) || \is_numeric($namespace)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($namespace) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($namespace) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    private function findAlternatives($name, $collection)
    {
        if (!(\is_array($collection) || $collection instanceof \Traversable)) {
            throw new \TypeError(__METHOD__ . '(): Argument #2 ($collection) must be of type iterable, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($collection) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
        if (!\is_string($name)) {
            if (!(\is_string($name) || \is_object($name) && \method_exists($name, '__toString') || (\is_bool($name) || \is_numeric($name)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($name) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($name) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        ...
        $phabelReturn = \array_keys($alternatives);
    ...
    public function setDefaultCommand($commandName, $isSingleCommand = \false)
    {
        if (!\is_string($commandName)) {
            if (!(\is_string($commandName) || \is_object($commandName) && \method_exists($commandName, '__toString') || (\is_bool($commandName) || \is_numeric($commandName)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($commandName) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($commandName) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $commandName = (string) $commandName;
            }
        }
        if (!\is_bool($isSingleCommand)) {
            if (!(\is_bool($isSingleCommand) || \is_numeric($isSingleCommand) || \is_string($isSingleCommand))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($isSingleCommand) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($isSingleCommand) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    {
        $phabelReturn = $this->singleCommand;
    ...
    private function splitStringByWidth($string, $width)
    {
        ...
        if (\false === ($encoding = \mb_detect_encoding($string, null, \true))) {
            $phabelReturn = \str_split($string, $width);
            if (!\is_array($phabelReturn)) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type array, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            }
            return $phabelReturn;

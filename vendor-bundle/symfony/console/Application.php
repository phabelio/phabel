...
class Application implements ResetInterface
{
    ...
    public function getNamespaces()
    {
        ...
        return \array_values(\array_unique(\Phabel\Target\Php74\Polyfill::array_filter($namespaces)));
    }
    /**
    ...
    public function all(string $namespace = null)
    {
        ...
        foreach ($this->commands as $name => $command) {
            if ($namespace === $this->extractNamespace($name, \Phabel\Target\Php71\Polyfill::substr_count($namespace, ':') + 1)) {
                $commands[$name] = $command;
            }
        }
        if ($this->commandLoader) {
            foreach ($this->commandLoader->getNames() as $name) {
                if (!isset($commands[$name]) && $namespace === $this->extractNamespace($name, \Phabel\Target\Php71\Polyfill::substr_count($namespace, ':') + 1) && $this->has($name)) {
                    $commands[$name] = $this->get($name);
                }
            }
        }
    ...
    private function splitStringByWidth(string $string, int $width) : array
    {
        ...
        $utf8String = \Phabel\Target\Php72\Polyfill::mb_convert_encoding($string, 'utf8', $encoding);
        $lines = [];
        $line = '';
        $offset = 0;

...
class TextDescriptor extends Descriptor
{
    ...
    protected function describeInputArgument(InputArgument $argument, array $options = [])
    {
        ...
        $totalWidth = isset($options['total_width']) ? $options['total_width'] : Helper::width($argument->getName());
    ...
    protected function describeInputDefinition(InputDefinition $definition, array $options = [])
    {
        ...
        if ($definition->getOptions()) {
            ...
            foreach ($definition->getOptions() as $option) {
                if (\strlen(NULL !== ($phabel_30ed5abe19edb800 = $option->getShortcut()) ? $phabel_30ed5abe19edb800 : '') > 1) {
                    $laterOptions[] = $option;
                    continue;
                }
                $this->writeText("\n");
    ...
    private function formatDefaultValue($default)
    {
        ...
        $phabelReturn = \str_replace('\\\\', '\\', \json_encode($default, \JSON_UNESCAPED_SLASHES | \JSON_UNESCAPED_UNICODE));
        if (!\is_string($phabelReturn)) {
            if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    private function getColumnWidth(array $commands)
    {
        ...
        $phabelReturn = $widths ? \max($widths) + 2 : 0;

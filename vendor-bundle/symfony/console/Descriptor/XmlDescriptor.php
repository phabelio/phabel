...
class XmlDescriptor extends Descriptor
{
    public function getInputDefinitionDocument(InputDefinition $definition)
    {
        ...
        $phabelReturn = $dom;
        if (!$phabelReturn instanceof \DOMDocument) {
            throw new \TypeError(__METHOD__ . '(): Return value must be of type DOMDocument, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
        return $phabelReturn;
    }
    public function getCommandDocument(Command $command, $short = \false)
    {
        if (!\is_bool($short)) {
    ...
    public function getApplicationDocument(Application $application, $namespace = null, $short = \false)
    {
        ...
        if (!\is_bool($short)) {
            if (!(\is_bool($short) || \is_numeric($short) || \is_string($short))) {
                throw new \TypeError(__METHOD__ . '(): Argument #3 ($short) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($short) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $short = (bool) $short;
            }
        }
    ...
    {
        $this->writeDocument($this->getCommandDocument($command, isset($options['short']) ? $options['short'] : \false));
    }
    /**
     * {@inheritdoc}
     */
    ...
    {
        $this->writeDocument($this->getApplicationDocument($application, isset($options['namespace']) ? $options['namespace'] : null, isset($options['short']) ? $options['short'] : \false));
    }
    /**
    ...
    private function getInputOptionDocument(InputOption $option)
    {
        ...
        $pos = \strpos(NULL !== ($phabel_aaa4c7c842e5280f = $option->getShortcut()) ? $phabel_aaa4c7c842e5280f : '', '|');
        if (\false !== $pos) {

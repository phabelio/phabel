...
class InputDefinition
{
    ...
    public function addArguments($arguments = [])
    {
        if (!(\is_array($arguments) || \is_null($arguments))) {
            throw new \TypeError(__METHOD__ . '(): Argument #1 ($arguments) must be of type ?array, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($arguments) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
        if (null !== $arguments) {
    ...
    public function hasNegation($name)
    {
        ...
        $phabelReturn = isset($this->negations[$name]);
    ...
    public function shortcutToName($shortcut)
    {
        ...
        $phabelReturn = $this->shortcuts[$shortcut];
    ...
    {
        if (!\is_string($negation)) {
            if (!(\is_string($negation) || \is_object($negation) && \method_exists($negation, '__toString') || (\is_bool($negation) || \is_numeric($negation)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($negation) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($negation) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
        ...
        $phabelReturn = $this->negations[$negation];
        if (!\is_string($phabelReturn)) {
            if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {

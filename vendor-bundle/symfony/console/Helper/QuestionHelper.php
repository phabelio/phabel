...
use Phabel\Symfony\Component\Console\Exception\MissingInputException;
...
use Phabel\Symfony\Component\Console\Input\StreamableInputInterface;
...
use function Phabel\Symfony\Component\String\s;
/**
...
class QuestionHelper extends Helper
{
    ...
    {
        if (!\is_string($tag)) {
            if (!(\is_string($tag) || \is_object($tag) && \method_exists($tag, '__toString') || (\is_bool($tag) || \is_numeric($tag)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($tag) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($tag) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $tag = (string) $tag;
            }
        }
    ...
    {
        if (!\is_string($entered)) {
            if (!(\is_string($entered) || \is_object($entered) && \method_exists($entered, '__toString') || (\is_bool($entered) || \is_numeric($entered)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($entered) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($entered) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
     ...
     * @throws RuntimeException In case the fallback is deactivated and the response cannot be hidden
     */
    private function getHiddenResponse(OutputInterface $output, $inputStream, $trimmable = \true)
    {
        if (!\is_bool($trimmable)) {
            if (!(\is_bool($trimmable) || \is_numeric($trimmable) || \is_string($trimmable))) {
                throw new \TypeError(__METHOD__ . '(): Argument #3 ($trimmable) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($trimmable) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    private function validateAttempts(callable $interviewer, OutputInterface $output, Question $question)
    {
        ...
        while (null === $attempts || $attempts--) {
            ...
            try {
                $phabel_b3555919d25e5efd = $question->getValidator();
                return $phabel_b3555919d25e5efd($interviewer());
            } catch (RuntimeException $e) {
                throw $e;
    ...
    {
        if ('php://stdin' !== (NULL !== ($phabel_376920cdae0760e0 = \stream_get_meta_data($inputStream)) && isset($phabel_376920cdae0760e0['uri']) ? $phabel_376920cdae0760e0['uri'] : null)) {
            $phabelReturn = \false;
        ...
        if (null !== self::$stdinIsInteractive) {
            $phabelReturn = self::$stdinIsInteractive;
            if (!\is_bool($phabelReturn)) {
                if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn) || \is_string($phabelReturn))) {
                    throw new \TypeError(__METHOD__ . '(): Return value must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                } else {
        ...
        if (\function_exists('stream_isatty')) {
            $phabelReturn = self::$stdinIsInteractive = \stream_isatty(\fopen('php://stdin', 'r'));
            if (!\is_bool($phabelReturn)) {
                if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn) || \is_string($phabelReturn))) {
                    throw new \TypeError(__METHOD__ . '(): Return value must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                } else {
        ...
        if (\function_exists('posix_isatty')) {
            $phabelReturn = self::$stdinIsInteractive = \posix_isatty(\fopen('php://stdin', 'r'));
            if (!\is_bool($phabelReturn)) {
                if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn) || \is_string($phabelReturn))) {
                    throw new \TypeError(__METHOD__ . '(): Return value must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                } else {
        ...
        if (!\function_exists('exec')) {
            $phabelReturn = self::$stdinIsInteractive = \true;
            if (!\is_bool($phabelReturn)) {
                if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn) || \is_string($phabelReturn))) {
                    throw new \TypeError(__METHOD__ . '(): Return value must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                } else {
        ...
        $phabelReturn = self::$stdinIsInteractive = 1 !== $status;
        if (!\is_bool($phabelReturn)) {
            if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn) || \is_string($phabelReturn))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    {
        if (!\is_int($cp)) {
            if (!(\is_bool($cp) || \is_numeric($cp))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($cp) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($cp) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $cp = (int) $cp;
            }
        }

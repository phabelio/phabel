...
class Table
{
    ...
    public function setHeaderTitle($title) : self
    {
        if (!\is_null($title)) {
            if (!\is_string($title)) {
                if (!(\is_string($title) || \Phabel\Target\Php72\Polyfill::is_object($title) && \method_exists($title, '__toString') || (\is_bool($title) || \is_numeric($title)))) {
    ...
    private function fillNextRows(array $rows, int $line) : array
    {
        ...
        foreach ($rows[$line] as $column => $cell) {
            if (null !== $cell && !$cell instanceof TableCell && !\is_scalar($cell) && !(\Phabel\Target\Php72\Polyfill::is_object($cell) && \method_exists($cell, '__toString'))) {
            ...
            if ($cell instanceof TableCell && $cell->getRowspan() > 1) {
                ...
                if (\strstr($cell, "\n")) {
                    $lines = \explode("\n", \str_replace("\n", "<fg=default;bg=default>\n</>", $cell));
                    $nbLines = \count($lines) > $nbLines ? \Phabel\Target\Php71\Polyfill::substr_count($cell, "\n") : $nbLines;

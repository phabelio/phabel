...
final class Grapheme
{
    ...
    public static function grapheme_extract($s, $size, $type = \GRAPHEME_EXTR_COUNT, $start = 0, &$next = 0)
    {
        ...
        if (!is_scalar($s)) {
            ...
            $next = \Phabel\Target\Php80\Polyfill::substr($s, $start);
            restore_error_handler();
            if ($hasError) {
                \Phabel\Target\Php80\Polyfill::substr($s, $start);
                $s = '';
            } else {
                $s = $next;
            }
        } else {
            $s = \Phabel\Target\Php80\Polyfill::substr($s, $start);
        }
        $size = (int) $size;
        $type = (int) $type;
        $start = (int) $start;
        ...
        do {
            if (\GRAPHEME_EXTR_COUNT === $type) {
                ...
                $size -= \Phabel\Target\Php80\Polyfill::iconv_strlen($s[$i], 'UTF-8//IGNORE');
            }
            if ($size >= 0) {
                $ret .= $s[$i];
            }
    ...
    private static function grapheme_position($s, $needle, $offset, $mode)
    {
        ...
        return false !== $needlePos ? self::grapheme_strlen(\Phabel\Target\Php80\Polyfill::substr($s, 0, $needlePos)) + $offset : false;
    }
}
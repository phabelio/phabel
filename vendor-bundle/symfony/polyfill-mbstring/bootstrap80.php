...
if (!function_exists('mb_substitute_character')) {
    ...
    {
        if (!(\is_int($substitute_character) || \is_string($substitute_character) || \is_null($substitute_character) || \is_null($substitute_character))) {
            if (!(\is_string($substitute_character) || \is_object($substitute_character) && \method_exists($substitute_character, '__toString') || (\is_bool($substitute_character) || \is_numeric($substitute_character)))) {
                if (!(\is_bool($substitute_character) || \is_numeric($substitute_character))) {
                    throw new \TypeError(__METHOD__ . '(): Argument #1 ($substitute_character) must be of type ?int|string|null, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($substitute_character) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                } else {

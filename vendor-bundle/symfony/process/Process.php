...
use Phabel\Symfony\Component\Process\Exception\InvalidArgumentException;
...
use Phabel\Symfony\Component\Process\Exception\ProcessSignaledException;
use Phabel\Symfony\Component\Process\Exception\ProcessTimedOutException;
use Phabel\Symfony\Component\Process\Exception\RuntimeException;
use Phabel\Symfony\Component\Process\Pipes\PipesInterface;
use Phabel\Symfony\Component\Process\Pipes\UnixPipes;
use Phabel\Symfony\Component\Process\Pipes\WindowsPipes;
/**
...
class Process implements \IteratorAggregate
{
    ...
    public function stop($timeout = 10, $signal = null)
    {
        if (!\is_float($timeout)) {
            if (!(\is_bool($timeout) || \is_numeric($timeout))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($timeout) must be of type float, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($timeout) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $timeout = (double) $timeout;
            }
        }
        if (!\is_null($signal)) {
            if (!\is_int($signal)) {
                if (!(\is_bool($signal) || \is_numeric($signal))) {
                    throw new \TypeError(__METHOD__ . '(): Argument #2 ($signal) must be of type ?int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($signal) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                } else {
                    $signal = (int) $signal;
                }
            }
    ...
    {
        if (!\is_string($line)) {
            if (!(\is_string($line) || \is_object($line) && \method_exists($line, '__toString') || (\is_bool($line) || \is_numeric($line)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($line) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($line) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $line = (string) $line;
            }
        }
    ...
    {
        $phabelReturn = $this->lastOutputTime;
    ...
    {
        if (!\is_bool($tty)) {
            if (!(\is_bool($tty) || \is_numeric($tty) || \is_string($tty))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($tty) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($tty) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $tty = (bool) $tty;
            }
        }
    ...
    {
        if (!\is_bool($bool)) {
            if (!(\is_bool($bool) || \is_numeric($bool) || \is_string($bool))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($bool) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($bool) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $bool = (bool) $bool;
            }
        }
    ...
    public function setWorkingDirectory($cwd)
    {
        if (!\is_string($cwd)) {
            if (!(\is_string($cwd) || \is_object($cwd) && \method_exists($cwd, '__toString') || (\is_bool($cwd) || \is_numeric($cwd)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($cwd) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($cwd) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $cwd = (string) $cwd;
            }
        }
    ...
    private function getDescriptors()
    {
        ...
        $phabelReturn = $this->processPipes->getDescriptors();
    ...
    protected function buildCallback(callable $callback = null)
    {
        if ($this->outputDisabled) {
            return function ($type, $data) use($callback) {
                $phabelReturn = null !== $callback && $callback($type, $data);
                if (!\is_bool($phabelReturn)) {
                    if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn) || \is_string($phabelReturn))) {
                        throw new \TypeError(__METHOD__ . '(): Return value must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                    } else {
    ...
    {
        if (!\is_string($caller)) {
            if (!(\is_string($caller) || \is_object($caller) && \method_exists($caller, '__toString') || (\is_bool($caller) || \is_numeric($caller)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($caller) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($caller) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    private function doSignal($signal, $throwException)
    {
        ...
        if (!\is_bool($throwException)) {
            if (!(\is_bool($throwException) || \is_numeric($throwException) || \is_string($throwException))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($throwException) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($throwException) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    private function requireProcessIsStarted($functionName)
    {
        if (!\is_string($functionName)) {
            if (!(\is_string($functionName) || \is_object($functionName) && \method_exists($functionName, '__toString') || (\is_bool($functionName) || \is_numeric($functionName)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($functionName) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($functionName) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    private function escapeArgument($argument)
    {
        if (!\is_null($argument)) {
            if (!\is_string($argument)) {
                if (!(\is_string($argument) || \is_object($argument) && \method_exists($argument, '__toString') || (\is_bool($argument) || \is_numeric($argument)))) {
                    throw new \TypeError(__METHOD__ . '(): Argument #1 ($argument) must be of type ?string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($argument) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                } else {
        ...
        if ('\\' !== \DIRECTORY_SEPARATOR) {
            $phabelReturn = "'" . \str_replace("'", "'\\''", $argument) . "'";
            if (!\is_string($phabelReturn)) {
                if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                    throw new \TypeError(__METHOD__ . '(): Return value must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                } else {
    ...
    private function replacePlaceholders($commandline, array $env)
    {
        if (!\is_string($commandline)) {
            if (!(\is_string($commandline) || \is_object($commandline) && \method_exists($commandline, '__toString') || (\is_bool($commandline) || \is_numeric($commandline)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($commandline) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($commandline) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {

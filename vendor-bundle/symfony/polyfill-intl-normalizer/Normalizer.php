 ...
 */
namespace Phabel\Symfony\Polyfill\Intl\Normalizer;
...
class Normalizer
{
    ...
    {
        if (!\is_string($s)) {
            if (!(\is_string($s) || \is_object($s) && \method_exists($s, '__toString') || (\is_bool($s) || \is_numeric($s)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($s) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($s) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $s = (string) $s;
            }
        }

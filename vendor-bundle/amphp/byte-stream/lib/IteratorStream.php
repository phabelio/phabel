    ...
    public function read() : Promise
    {
        ...
        $this->iterator->advance()->onResolve(function ($error, $hasNextElement) use($deferred) {
            ...
            if ($error) {
                ...
                if (!\is_string($chunk)) {
                    $this->exception = new StreamException(\sprintf("Unexpected iterator value of type '%s', expected string", \Phabel\Target\Php72\Polyfill::is_object($chunk) ? \get_class($chunk) : \gettype($chunk)));

...
abstract class Driver
{
    ...
    private function tick()
    {
        ...
        foreach ($this->deferQueue as $watcher) {
            ...
            try {
                $phabel_56185a565d967e53 = $watcher->callback;
                /** @var mixed $result */
                $result = $phabel_56185a565d967e53($watcher->id, $watcher->data);
                if ($result === null) {
                    continue;
                }
     ...
     * @return void
     */
    protected abstract function dispatch($blocking);
    /**
     * Stop the event loop.
     *
    ...
    protected function error(\Throwable $exception)
    {
        ...
        $phabel_6170bbb6d7be24ff = $this->errorHandler;
        $phabel_6170bbb6d7be24ff($exception);
    }
    /**
    ...
    public function now()
    {
        $phabelReturn = (int) (\microtime(\true) * self::MILLISEC_PER_SEC);
    ...
    public function getInfo()
    {
        ...
        $phabelReturn = ["enabled_watchers" => $watchers, "defer" => $defer, "delay" => $delay, "repeat" => $repeat, "on_readable" => $onReadable, "on_writable" => $onWritable, "on_signal" => $onSignal, "running" => (bool) $this->running];

...
class NativeDriver extends Driver
{
    ...
    public function onSignal($signo, callable $callback, $data = null)
    {
        ...
        $phabelReturn = parent::onSignal($signo, $callback, $data);
    ...
    protected function dispatch($blocking)
    {
        ...
        while ($watcher = $this->timerQueue->extract($now)) {
            ...
            try {
                $phabel_a422fc52460a9b50 = $watcher->callback;
                // Execute the timer.
                $result = $phabel_a422fc52460a9b50($watcher->id, $watcher->data);
                if ($result === null) {
                    continue;
                }
    ...
    private function selectStreams(array $read, array $write, $timeout)
    {
        if (!\is_int($timeout)) {
            if (!(\is_bool($timeout) || \is_numeric($timeout))) {
                throw new \TypeError(__METHOD__ . '(): Argument #3 ($timeout) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($timeout) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $timeout = (int) $timeout;
            }
        }
        ...
        if (!empty($read) || !empty($write)) {
            ...
            foreach ($read as $stream) {
                ...
                foreach ($this->readWatchers[$streamId] as $watcher) {
                    ...
                    try {
                        $phabel_668b107f3be16641 = $watcher->callback;
                        $result = $phabel_668b107f3be16641($watcher->id, $stream, $watcher->data);
                        if ($result === null) {
                            continue;
                        }
                        if ($result instanceof \Generator) {
            ...
            foreach ($write as $stream) {
                ...
                foreach ($this->writeWatchers[$streamId] as $watcher) {
                    ...
                    try {
                        $phabel_d881bf449cdca21d = $watcher->callback;
                        $result = $phabel_d881bf449cdca21d($watcher->id, $stream, $watcher->data);
                        if ($result === null) {
                            continue;
                        }
                        if ($result instanceof \Generator) {
    ...
    private function getTimeout()
    {
        ...
        $phabelReturn = $expiration > 0 ? $expiration : 0;
        if (!\is_int($phabelReturn)) {
            if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    private function handleSignal($signo)
    {
        ...
        foreach ($this->signalWatchers[$signo] as $watcher) {
            ...
            try {
                $phabel_cb9626d52af40a29 = $watcher->callback;
                $result = $phabel_cb9626d52af40a29($watcher->id, $signo, $watcher->data);
                if ($result === null) {
                    continue;
                }
                if ($result instanceof \Generator) {

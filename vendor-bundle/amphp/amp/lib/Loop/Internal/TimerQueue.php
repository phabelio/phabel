...
namespace Phabel\Amp\Loop\Internal;
...
final class TimerQueue
{
    ...
    {
        if (!\is_int($node)) {
            if (!(\is_bool($node) || \is_numeric($node))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($node) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($node) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $node = (int) $node;
            }
        }
    ...
    {
        if (!\is_int($left)) {
            if (!(\is_bool($left) || \is_numeric($left))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($left) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($left) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $left = (int) $left;
            }
        }
        if (!\is_int($right)) {
            if (!(\is_bool($right) || \is_numeric($right))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($right) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($right) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $right = (int) $right;
            }
        }
     ...
     */
    public function extract($now)
    {
        if (!\is_int($now)) {
            if (!(\is_bool($now) || \is_numeric($now))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($now) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($now) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $now = (int) $now;
            }
        }
     ...
     * @return void
     */
    private function removeAndRebuild($node)
    {
        if (!\is_int($node)) {
            if (!(\is_bool($node) || \is_numeric($node))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($node) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($node) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());

...
final class SharedMemoryParcel implements Parcel
{
     ...
     */
    public static function create(string $id, $value, int $size = 8192, int $permissions = 0600, $serializer = null) : self
    {
     ...
     * @throws SharedMemoryException
     */
    public static function use(string $id, $serializer = null) : self
    {
        if (!($serializer instanceof Serializer || \is_null($serializer))) {
     ...
     * @param Serializer|null $serializer
     */
    private function __construct(string $id, $serializer = null)
    {
        if (!($serializer instanceof Serializer || \is_null($serializer))) {
            throw new \TypeError(__METHOD__ . '(): Argument #2 ($serializer) must be of type ?Serializer, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($serializer) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
    ...
    private function getHeader() : array
    {
        ...
        return \Phabel\Target\Php71\Polyfill::unpack('Cstate/Lsize/Spermissions', $data);
    }
    /**
    ...
    {
        return \abs(\Phabel\Target\Php71\Polyfill::unpack("l", \md5($id, \true))[1]);
    }
}

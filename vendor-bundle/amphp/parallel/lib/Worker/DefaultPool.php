...
use Phabel\Amp\Parallel\Context\StatusError;
...
final class DefaultPool implements Pool
{
     ...
     * @throws \Error
     */
    public function __construct($maxSize = self::DEFAULT_MAX_SIZE, WorkerFactory $factory = null)
    {
        if (!\is_int($maxSize)) {
            if (!(\is_bool($maxSize) || \is_numeric($maxSize))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($maxSize) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($maxSize) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $maxSize = (int) $maxSize;
            }
        }
    ...
    {
        $phabelReturn = $this->idleWorkers->count() > 0 || $this->workers->count() === 0;
        if (!\is_bool($phabelReturn)) {
            if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn) || \is_string($phabelReturn))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    {
        $phabelReturn = $this->workers->count();
        if (!\is_int($phabelReturn)) {
            if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    {
        $phabelReturn = $this->idleWorkers->count();
        if (!\is_int($phabelReturn)) {
            if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn))) {
                throw new \TypeError(__METHOD__ . '(): Return value must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
    ...
    public function enqueue(Task $task)
    {
        ...
        $promise->onResolve(function () use($worker) {
            $phabel_952f19190862811f = $this->push;
            $phabel_952f19190862811f($worker);
        });
    ...
    public function shutdown()
    {
        ...
        $phabelReturn = $this->exitStatus = Promise\all($shutdowns);
        if (!$phabelReturn instanceof Promise) {
            throw new \TypeError(__METHOD__ . '(): Return value must be of type Promise, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
        return $phabelReturn;
    ...
    {
        $phabelReturn = new Internal\PooledWorker($this->pull(), $this->push);

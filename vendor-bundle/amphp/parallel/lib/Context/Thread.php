...
final class Thread implements Context
{
    ...
    public function start() : Promise
    {
        ...
        $this->oid = \getmypid();
        $sockets = @\stream_socket_pair(\Phabel\Target\Php71\Polyfill::stripos(\PHP_OS, "win") === 0 ? \STREAM_PF_INET : \STREAM_PF_UNIX, \STREAM_SOCK_STREAM, \STREAM_IPPROTO_IP);
        if ($sockets === \false) {
    ...
    public function receive() : Promise
    {
        ...
        return call(function () : \Generator {
            ...
            if ($data instanceof ExitResult) {
                $data = $data->getResult();
                throw new SynchronizationError(\sprintf('Thread process unexpectedly exited with result of type: %s', \Phabel\Target\Php72\Polyfill::is_object($data) ? \get_class($data) : \gettype($data)));
            }
            return $data;
        });
    }

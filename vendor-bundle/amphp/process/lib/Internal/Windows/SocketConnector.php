...
final class SocketConnector
{
    ...
    private function failClientHandshake($socket, $code)
    {
        if (!\is_int($code)) {
    ...
    public function failHandleStart(Handle $handle, $message, ...$args)
    {
     ...
     * @return string|null
     */
    private function readDataFromPendingClient($socket, $length, PendingSocketClient $state)
    {
        if (!\is_int($length)) {

...
class IssetExpressionFixer extends Plugin
{
    ...
    {
        $phabelReturn = new ArrayDimFetch(self::callPoly('returnMe', new Ternary($node, self::toLiteral([0]), self::toLiteral([]))), new LNumber(0));
        ...
        foreach ($isset->vars as $key => &$var) {
            ...
            switch ($class = \get_class($workVar)) {
                ...
                case StaticPropertyFetch::class:
                    $workVar = $this->wrapBoolean(self::callPoly('staticExists', $workVar->class, $workVar->name instanceof VarLikeIdentifier ? new String_($workVar->name->name) : $workVar->name, new LNumber(1)));
                    break;
                case ClassConstFetch::class:
                    $workVar = $this->wrapBoolean(self::callPoly('staticExists', $workVar->class, new String_($workVar->name->name), new LNumber(0)));
                    break;
                case Variable::class:
    ...
    public static function staticExists($class, $property, $propertyOrConstant)
    {
        ...
        $classCaller = null !== ($phabel_f95cdaed748552c2 = \debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 2)) && isset($phabel_f95cdaed748552c2[1]['class']) ? $phabel_f95cdaed748552c2[1]['class'] : '';

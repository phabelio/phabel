...
class Polyfill extends Plugin
{
    ...
    public static function date_sunset(
        int $timestamp,
        int $returnFormat = SUNFUNCS_RET_STRING,
        ?float $latitude = null,
        ?float $longitude = null,
    ...
    public static function session_set_cookie_params(
        ...
        return \is_array($lifetime_or_options)
            ? \session_set_cookie_params($lifetime_or_options)
            : \session_set_cookie_params(\array_filter(['lifetime' => $lifetime_or_options, 'path' => $path, 'domain' => $domain, 'secure' => $secure, 'httponly' => $httponly]));
    }
